...
export class ChartGenerationOperation implements StreamingOperation<AgentResponse> {
  ...
  async execute(progressCallback: EventCallback): Promise<AgentResponse> {
    ...
    try {
      // Run the chart generation state machine with real-time progress
      ...
      if (result.errors.length > 0) {
        const errorMessage = `Chart generation failed: ${result.errors.join(', ')}`;
      ...
      const errorMessage = `Chart generation failed: ${error instanceof Error ? error.message : String(error)}`;
